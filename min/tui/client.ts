...
export async function sendPrompt(sessionId: string, content: string): Promise<Response> {
...
export async function contextAdd(sessionId: string, path: string, readonly = false): Promise<void> {
  await req("POST", "/context/add", { session_id: sessionId, path, readonly })
}

export async function contextDrop(sessionId: string, path: string): Promise<void> {
