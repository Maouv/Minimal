...
import { pushMessage, setState, state } from "../state.ts";
...
export function InputBox() {
	...
	async function handleSubmit(value: string) {
		...
		const isEditWithPrompt = /^\/(edit-block|edit-udiff|edit-whole)\s+\S/.test(
			raw,
		);
	...
	async function handleInput(value: string) {
		...
		if (value.startsWith("/") && !value.includes(" ")) {
			const matches = SLASH_COMMANDS.filter((c) => c.cmd.startsWith(value)).map(
				(c) => ({ label: c.cmd, desc: c.desc, value: `${c.cmd} ` }),
			);
			setAcMode("command");
			setAcItems(matches);
			setAcSelected(0);
		...
		if (addCmd) {
			...
			const matches = rawEntries
				...
				.filter(
					(e) => filterName === "" || e.name.toLowerCase().includes(filterName),
				)
				.slice(0, 12)
				.map((e) => ({
		...
		if (value.startsWith("/init ") && !value.includes("--")) {
			...
			const allDirs = [
				{ label: dirCacheRoot, desc: "./", value: "/init ./" },
				...dirCache.map((d) => {
			...
			const matches = allDirs
				.filter(
					(d) =>
						filter === "" ||
						d.desc.toLowerCase().includes(filter) ||
			...
			>
				...
				>
					<box
						...
						marginBottom={1}
					>
						<text fg={isDisabled() ? C.gray2 : C.blue} marginRight={1}>
							✦
						</text>
						<input
							ref={setInputRef}
							...
							placeholder={
								isDisabled()
									? ""
									: 'Ask anything... "Whats the tech stack of this project?"'
							}
							...
							focused={!isDisabled()}
							onInput={(val: string) => {
								if (!isDisabled()) handleInput(val);
							}}
							onSubmit={() => {
								if (!isDisabled()) handleSubmit(inputRef?.value ?? "");
							}}
						/>
					</box>
