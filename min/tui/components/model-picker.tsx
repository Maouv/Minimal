...
export function ModelPicker(props: Props) {
	...
	const [activeProvider, setActiveProvider] = createSignal<Provider | null>(
		null,
	);
	...
	const inputLabel = createMemo(() => {
		switch (phase()) {
			case "new-baseurl":
				return "Base URL";
			case "new-apikey":
				return `API Key  ${newBaseUrl()}`;
			case "model-select":
				return manualMode() ? `Model ID  ${error()}` : "";
			default:
				return "";
		}
	});
	...
	const inputPlaceholder = createMemo(() => {
		switch (phase()) {
			...
			case "new-apikey":
				return "sk-or-v1-...";
			case "model-select":
				return manualMode() ? "openai/gpt-4o" : "filter model...";
			default:
				return "";
		}
	});
	...
	const listItems = () => {
		const existing = providers().map((p) => ({
			label: p.name,
			desc: p.base_url,
			isNew: false,
			provider: p,
	...
	const filteredModels = () => {
		...
		const all = modelList();
		return (q ? all.filter((m) => m.toLowerCase().includes(q)) : all).slice(
			0,
			MAX_VISIBLE,
		);
	};
	...
	useKeyboard((key) => {
		...
		if (ph === "provider-list") {
			...
			if (key.name === "down") {
				key.preventDefault();
				setSelIdx((s) => Math.min(items.length - 1, s + 1));
				return;
			}
			if (key.name === "return") {
				key.preventDefault();
		...
		if (ph === "model-select" && !manualMode()) {
			...
			if (key.name === "down") {
				key.preventDefault();
				setSelIdx((s) => Math.min(models.length - 1, s + 1));
				return;
			}
			if (key.name === "return") {
				key.preventDefault();
	...
	async function _probe(baseUrl: string, apiKey: string) {
		...
		setPhase("loading");
		setLoadingMsg(
			`Fetching models${providerName ? ` from ${providerName}` : ""}...`,
		);
	...
	async function _handleSubmit(val: string) {
		...
		if (ph === "new-baseurl") {
			...
			if (!url) return;
			setNewBaseUrl(url);
			setNewName(_deriveNameFromUrl(url));
				...
				<Show when={phase() === "model-select" && !manualMode()}>
					...
					<Show when={modelList().length > MAX_VISIBLE}>
						<box height={1} paddingLeft={2} paddingBottom={1}>
							<text
								fg={C.gray2}
							>{`${modelList().length - filteredModels().length} more — ketik untuk filter`}</text>
						</box>
					</Show>
				</Show>
