...
def apply_edits(
    response: str, files: dict[str, str], mode: EditMode
) -> list[EditResult]:
    """
...
def _apply_editblock(response: str, files: dict[str, str]) -> list[EditResult]:
    from vendor.search_replace import (
    ...
    try:
        ...
        return [
            EditResult(
                file="", original="", updated="", diff="", success=False, error=str(e)
            )
        ]
        ...
        if not matched_path:
            results.append(
                EditResult(
                    ...
                    diff="",
                    success=False,
                    error=f"File not in context: {fname}",
                )
            )
            continue
    ...
    if not edits:
        ...
        return [
            EditResult(
                ...
                diff="",
                success=False,
                error="Could not parse whole file response",
            )
        ]
